class BrowserStoreElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.slotElement.addEventListener('slotchange', this.handleSlotChange, { signal: this.abortController?.signal });
    ...
    for (const key in data) {
      ...
      if (Array.isArray(value)) {
        for (const item of value) {
          ...
          this.form?.querySelector(`[name="${key}"][value="${item}"]`)?.setAttribute('checked', '');
        }
      } else {
        this.form?.querySelector(`[name="${key}"][value="${value}"]`)?.setAttribute('checked', '');
      }
    }
  }
